package fiji.plugins.small.skel;
...
public class AnalyzeSkeleton
{
	...
	private List< Double > shortestPathList;
	/** list containing longest shortest path points (one per tree) */
	private List< List< Point > > shortestPathPoints; 
	...
	private List < List <Point> > endPointsTree;
	/** list of junction voxels in every tree */
	private List < List <Point> > junctionVoxelTree;
	/** list of special slab coordinates where circular tree starts */
	private List < List <Point> > startingSlabTree;
	...
	private List <Point> listOfEndPoints = null;
	/** list of junction coordinates in the entire image */
	private List <Point> listOfJunctionVoxels = null;
	/** list of slab coordinates in the entire image */
	private List <Point> listOfSlabVoxels = null;
	/** list of slab coordinates in the entire image */
	private List <Point> listOfStartingSlabVoxels = null;
	
	/** list of groups of junction voxels that belong to the same tree junction (in every tree) */
	private List < List < List <Point> > > listOfSingleJunctions;
	...
	private List<Point> slabList = null;
			...
			Roi roi)
	{
		...
		if(shortPath)
		{
			...
			{							
				shortestPathPoints.add( new ArrayList<Point>());
				// Warshall algorithm including tag positions
				this.shortestPath = warshallAlgorithm(this.graph[i], shortestPathPoints.get(i));
	 ...
	 */
	public List<List<Point>> getShortestPathPoints()
	{
	...
	public void processSkeleton(ImageStack inputImage2) 
	{
		...
		if(this.numOfTrees == 1)
		{
			...
			this.endPointsTree.set(0, this.listOfEndPoints);
			this.numberOfEndPoints[0] = this.listOfEndPoints.size();
			this.junctionVoxelTree.set(0, this.listOfJunctionVoxels);
			this.numberOfJunctionVoxels[0] = this.listOfJunctionVoxels.size();
			this.startingSlabTree.set(0, this.listOfStartingSlabVoxels);
		}
			...
			final int pruningMode) 
	{
		...
		for(int iTree = 0 ; iTree < this.numOfTrees; iTree ++)
		{
			...
			if(this.startingSlabTree.get(iTree).size() == 1)
			{
				setPixel(inputImage, this.startingSlabTree.get(iTree).get(0),(byte) 0);
				pruned = true;
			}
	...
	private void divideVoxelsByTrees(ImageStack treeIS) 
	{
		...
		for(int i = 0; i < this.totalNumberOfEndPoints; i++)
		{
			...
			this.endPointsTree.get((int)getFloatPixel( treeIS, p ) - 1 ).add(p);
		}
		...
		for(int i = 0; i < this.totalNumberOfJunctionVoxels; i++)
		{
			...
			this.junctionVoxelTree.get( (int)getFloatPixel( treeIS, p ) - 1 ).add(p);;
		}
		...
		for(int i = 0; i < this.listOfStartingSlabVoxels.size(); i++)
		{
			...
			this.startingSlabTree.get( (int)getFloatPixel( treeIS, p ) - 1 ).add(p);;
		}
		
		// Assign number of end points and junction voxels per tree
		for(int iTree = 0; iTree < this.numOfTrees; iTree++)
		{
			this.numberOfEndPoints[iTree] = this.endPointsTree.get(iTree).size();
			this.numberOfJunctionVoxels[iTree] = this.junctionVoxelTree.get(iTree).size();
		}
	...
	private void initializeTrees()
	{
		...
		for(int i = 0; i < this.numOfTrees; i++)
		{
			this.endPointsTree.add( new ArrayList <Point>());
			this.junctionVoxelTree.add( new ArrayList <Point>());
			this.startingSlabTree.add( new ArrayList <Point>());
			this.listOfSingleJunctions.add( new ArrayList < List <Point> > ());
		}
	...
	private void visitSkeleton(ImageStack taggedImage, ImageStack treeImage, int currentTree) 
	{
		...
		{									
			final Point endPointCoord = this.endPointsTree.get(iTree).get(i);
		...
		if(this.numberOfEndPoints[iTree] == 0 && this.junctionVoxelTree.get(iTree).size() > 0)
		...
		{			
			...
			final Point startCoord = this.startingSlabTree.get(iTree).get(0);
	...
	private void groupJunctions(ImageStack treeIS) 
	{
		...
		for (int iTree = 0; iTree < this.numOfTrees; iTree++)
		{
			...
			{
				Point pi = this.junctionVoxelTree.get(iTree).get(i);
				
				if(! isVisited(pi))
					fusionNeighborJunction(pi, this.listOfSingleJunctions.get(iTree));
			}
		}		
		...
		{
			if(debug)
				IJ.log("this.listOfSingleJunctions["+iTree+"].size() = " + this.listOfSingleJunctions.get(iTree).size());
			
			this.numberOfJunctions[iTree] = this.listOfSingleJunctions.get(iTree).size();
			
			// Create array of junction vertices for the graph
			this.junctionVertex[iTree] = new Vertex[this.listOfSingleJunctions.get(iTree).size()];
			
			for(int j = 0 ; j < this.listOfSingleJunctions.get(iTree).size(); j++)
			{
				final List<Point> list = this.listOfSingleJunctions.get(iTree).get(j);
	 ...
	 * @param list list of single junctions
	 */
	private void fusionNeighborJunction(Point startingPoint, List<List<Point>> list) 
	{
	...
	private void calculateTripleAndQuadruplePoints() 
	{
		...
		{			
			...
			for(int i = 0; i < this.numberOfJunctions[iTree]; i ++)
			{
				...
				List <Point> groupOfJunctions = this.listOfSingleJunctions.get(iTree).get(i);
	 ...
	 * @param shortestPathPoints2 list to store the longest shortest path points
	 * @return longest shortest path length
	 * @author Huub Hovens
	 */
	private double warshallAlgorithm(Graph graph, List<Point> shortestPathPoints2)
	{
		// local fields
		/** vertex 1 of an edge */
		Vertex v1 = null;
		...
		reconstructPath(predecessorMatrix, a, b, edgeList, vertexList, shortestPathPoints2);
	 ...
	 * @param shortestPathPoints2 contains points of the longest shortest path for each graph
	 * @author Huub Hovens
	 */
	private void reconstructPath(
			...
			List<Point> shortestPathPoints2)
	{
		...
		while (b != a)
		{
			...
			{
				if( ! shortestPathPoints2.contains( p ))
				{
					shortestPathPoints2.add(p);
					//setPixel(this.shortPathImage, p.x, p.y, p.z, SHORTEST_PATH);
				}
		...
		if (shortestPathPoints2.size() != 0)
		{
			this.spx = shortestPathPoints2.get(0).x;
			this.spy = shortestPathPoints2.get(0).y;
			this.spz = shortestPathPoints2.get(0).z;
		}		
